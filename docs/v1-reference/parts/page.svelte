<script lang="ts">
	...
	let hasActiveFilters = $derived(Boolean(typeFilter || startDate || endDate || retractedFilter));
	...
	let currentRevision = $derived(data.ipRevisions.find((r) => r.isCurrent));

	// Inspection config editing state
	let editingInspectionConfig = $state(false);
	let sampleSizeInput = $state(data.sampleSize);
	let percentAcceptedInput = $state(data.percentAccepted);

	$effect(() => {
		if (!editingInspectionConfig) {
			sampleSizeInput = data.sampleSize;
			percentAcceptedInput = data.percentAccepted;
		}
	});
...
<div class="space-y-6">
	...
	{#if activeTab === 'overview'}
		...
		<!-- IP Revision History -->
		{#if item.inspectionPathway === 'ip' && data.partDefinitionId}
			<IpRevisionHistory
				revisions={data.ipRevisions}
				partDefinitionId={data.partDefinitionId}
				ipError={form?.ipError}
				ipSuccess={form?.ipSuccess}
			/>

			<!-- IP Form Definition Editor (only when a current revision exists) -->
			{#if currentRevision}
				<IpFormDefinitionEditor
					revisionId={currentRevision.id}
					formDefinition={currentRevision.formDefinition}
					formDefError={form?.formDefError}
					formDefSuccess={form?.formDefSuccess}
				/>
			{/if}

			<!-- Inspection Configuration -->
			<TronCard>
				<h3 class="tron-text-primary mb-4 text-lg font-semibold">Inspection Configuration</h3>

				{#if form?.inspectionConfigSuccess}
					<div
						class="mb-4 rounded border border-[var(--color-tron-green)] bg-[color-mix(in_srgb,var(--color-tron-green)_10%,transparent)] px-4 py-2 text-[var(--color-tron-green)]"
					>
						Inspection configuration updated successfully.
					</div>
				{/if}
				{#if form?.inspectionConfigError}
					<div
						class="mb-4 rounded border border-[var(--color-tron-error)] bg-[color-mix(in_srgb,var(--color-tron-error)_10%,transparent)] px-4 py-2 text-[var(--color-tron-error)]"
					>
				...
				{#if editingInspectionConfig}
					...
					>
						<input type="hidden" name="partDefinitionId" value={data.partDefinitionId} />
						<div class="grid gap-4 sm:grid-cols-2">
							<div>
								<label for="sampleSize" class="tron-text-muted mb-1 block text-sm">
									Sample Size
								</label>
								<input
									id="sampleSize"
									...
									min="1"
									class="w-full rounded border border-[var(--color-tron-border)] bg-[var(--color-tron-bg-secondary)] px-3 py-2 font-mono text-[var(--color-tron-text)]"
								/>
								<p class="tron-text-muted mt-1 text-xs">
									Number of samples to inspect per lot
								</p>
							</div>
							<div>
								<label for="percentAccepted" class="tron-text-muted mb-1 block text-sm">
									Percent Accepted (%)
								</label>
								<input
									id="percentAccepted"
								...
								/>
								<p class="tron-text-muted mt-1 text-xs">
									Minimum pass rate required for acceptance (0-100)
								</p>
							</div>
						</div>
						<div class="mt-4 flex gap-2">
							<button
								type="submit"
								class="rounded bg-[var(--color-tron-cyan)] px-4 py-2 text-sm font-semibold text-black"
							>
								Save
							</button>
							<button
								...
								class="tron-text-muted px-4 py-2 text-sm hover:text-[var(--color-tron-text)]"
							>
								Cancel
							</button>
						</div>
				...
				{:else}
					<dl class="space-y-3">
						<div class="flex justify-between">
							<dt class="tron-text-muted">Sample Size</dt>
							<dd class="tron-text-primary font-mono">{data.sampleSize}</dd>
						</div>
						<div class="flex justify-between">
							<dt class="tron-text-muted">Percent Accepted</dt>
							<dd class="tron-text-primary font-mono">{data.percentAccepted}%</dd>
						</div>
					</dl>
					<button
						type="button"
						onclick={() => (editingInspectionConfig = true)}
						class="mt-4 rounded border border-[var(--color-tron-cyan)] px-4 py-2 text-sm text-[var(--color-tron-cyan)] hover:bg-[color-mix(in_srgb,var(--color-tron-cyan)_10%,transparent)]"
					>
						Edit Configuration
					</button>
				{/if}
		...
		<TronCard>
			<div class="mb-4 flex items-center justify-between">
				...
				<span class="text-xs text-[var(--color-tron-text-secondary)]">
					{data.inventoryTransactions.length} transaction{data.inventoryTransactions.length !== 1
						? 's'
						: ''}
				</span>
			</div>
								...
								<tr class={txn.retractedAt ? 'opacity-50' : ''}>
									...
									<td>
										{#if !txn.retractedAt && txn.transactionType === 'deduction'}
											...
											{:else}
												<button class="retract-btn" onclick={() => (retractingId = txn.id)}>
													Retract
												</button>
											{/if}
										{:else}
