			...
			<table class="tron-table">
				...
				<tbody>
					{#each revisions as rev (rev.id)}
						<tr>
							<td class="font-mono">{rev.revisionNumber}</td>
							<td class="tron-text-muted text-sm">{rev.uploadedByName ?? 'Unknown'}</td>
							<td class="text-sm">{formatDateTime(rev.uploadedAt)}</td>
							<td class="tron-text-muted max-w-xs truncate text-sm">{rev.changeNotes || '—'}</td>
							<td>
								{#if rev.isCurrent}
