				...
				<tbody>
					{#each data.lots as lot}
						<tr class="tron-border-b hover:bg-white/5 border-b">
							<td class="px-4 py-3">
								<a href="/spu/lots/{lot.lotId}" class="tron-link font-mono text-xs">
									{lot.lotId}
								</a>
							</td>
							<td class="tron-text px-4 py-3 font-mono text-xs">{lot.partNumber ?? '—'}</td>
							<td class="tron-text px-4 py-3">{lot.partName ?? '—'}</td>
