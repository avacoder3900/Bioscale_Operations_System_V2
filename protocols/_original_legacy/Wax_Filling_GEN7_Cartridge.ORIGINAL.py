...
def run(protocol: protocol_api.ProtocolContext):
    ...
    try:
        ...
        def pick_up_and_calibrate_tip():
            ...
            xOffset = round(x_pos - shift -150.536 + offset['x'], 1)
            x_pos = 134.01
            y_pos = 165.747
            pipette.move_to(types.Location(types.Point(x=x_pos, y=y_pos, z=z_cal), carriage), force_direct=True, speed=20)
            limit_reached = False
            ...
            pipette.move_to(types.Location(types.Point(x=127.181, y=174.247, z=z_cal), carriage), force_direct=True, speed=20)
            pipette.move_to(types.Location(types.Point(x=127.181, y=174.247, z=z_cal + 20), carriage), force_direct=True, speed=20)
