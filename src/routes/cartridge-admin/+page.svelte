<script lang="ts">
	...
	function stageLabel(stage: string): string {
		if (stage === 'backing') return 'Backed'; // BUCKET-SYSTEM_PLAN v2 §2: last bucket stage
		return stage.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
	}

	const stageColors: Record<string, string> = {
		raw: 'bg-slate-900/50 text-slate-300 border-slate-500/30',
		unpressed: 'bg-sky-900/50 text-sky-300 border-sky-500/30',
		pressed: 'bg-amber-900/50 text-amber-300 border-amber-500/30',
		...
		reagent_qc: 'bg-amber-900/50 text-amber-300 border-amber-500/30',
		reagent_ready: 'bg-green-900/50 text-green-300 border-green-500/30',
		reagent_rejected: 'bg-red-900/50 text-red-300 border-red-500/30',
