 ...
 *   Request Format: JSON
 *
 * One event family is handled specially: magnetometer sweep chunks. A sweep is
 * far larger than a Particle event, so firmware v98 uploads it as a sequence of
 * ~1 KB publishes. Those are buffered (see SweepUpload) until the whole sweep
 * is present, then turned into a ValidationSession identical in shape to the
 * one the bench-side agent-API ingest produces.
 */
...
function isSweepChunkEvent(eventName: unknown): boolean {
...
async function handleSweepChunk(deviceId: string, rawData: unknown, at: Date) {
...
export const POST: RequestHandler = async ({ request }) => {
	...
	// Device heartbeat is the same for every event, sweep chunks included.
	await ParticleDevice.updateOne(
		{ particleDeviceId: deviceId },
		{ $set: { lastHeardAt: at, status: 'online' } }
	);
