...
async function runDigest(request: Request) {
	...
	// The optics → SPU write-back is NOT run here any more (Jacob, 2026-09-15):
	// what a passing optical test means is still being worked out, so nothing
	// judges optics on a timer. It is a manual button on /validation
	// (?/syncOptics) and the sync-optics agent/MCP tools.
