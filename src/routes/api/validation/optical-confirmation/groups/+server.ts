...
import { connectDB, ValidationGroup, AuditLog, generateId } from '$lib/server/db';
...
export const GET: RequestHandler = async ({ url, locals }) => {
	...
	const groups = await ValidationGroup.find(filter).select('name description color').sort({ name: 1 }).limit(20).lean();
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	const existing = await ValidationGroup.findOne({ name: trimmed }).lean();
	if (existing) return json({ group: JSON.parse(JSON.stringify(existing)), created: false });

	const group = await ValidationGroup.create({
		_id: generateId(),
		name: trimmed,
	...
	await AuditLog.create({
		tableName: 'validation_groups', recordId: group._id, action: 'INSERT',
