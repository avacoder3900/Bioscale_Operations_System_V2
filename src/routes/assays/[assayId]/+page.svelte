<script lang="ts">
	...
	async function handleRestoreVersion(versionNumber: number) {
		const res = await fetch(`/assays/${data.assay.assayId}/versions`, {
			method: 'POST',
...
<div class="mx-auto max-w-7xl space-y-6 p-4">
	...
	<div class="flex items-start justify-between">
		...
		<div class="flex gap-2">
			{#if data.canWrite}
				<a
					href="/assays/{data.assay.assayId}/edit"
					class="tron-button"
				...
				<a
					href="/assays/{data.assay.assayId}/assign"
					class="tron-button"
					style="min-height: 44px"
				>
			...
			{/if}
			<a
				href="/assays/export?format=json&assayId={data.assay.assayId}"
				class="tron-button"
				style="min-height: 44px"
			>
				Export JSON
