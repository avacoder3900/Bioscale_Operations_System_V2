...
<div class="space-y-6">
	...
	{#if data.cartridge}
		<div class="rounded-lg border border-[var(--color-tron-border)] bg-[var(--color-tron-bg-secondary)] p-4">
			<div class="flex items-center gap-1 overflow-x-auto">
				{#each CARTRIDGE_PHASES as phaseVal, i}
					{@const isCurrent = data.cartridge?.currentPhase === phaseVal}
					{@const phaseIndex = CARTRIDGE_PHASES.indexOf(
						(data.cartridge?.currentPhase as (typeof CARTRIDGE_PHASES)[number]) ?? 'backing'
					)}
	...
	<div class="flex flex-wrap gap-1">
		...
		{#each data.phases as phaseTab}
			{@const count = data.images.filter(
				(img) => (img.cartridge_tag?.phase || 'untagged') === phaseTab
			).length}
			<button
				onclick={() => (activePhase = phaseTab)}
				class="rounded-lg px-3 py-1.5 text-sm font-medium transition-colors
					{activePhase === phaseTab
						? 'bg-[var(--color-tron-cyan)] text-[var(--color-tron-bg-primary)]'
						: 'text-[var(--color-tron-text-secondary)] hover:bg-[var(--color-tron-bg-tertiary)]'}"
			>
				{phaseTab.replace(/_/g, ' ')} ({count})
			</button>
		{/each}
	</div>
	...
	{#if filteredImages.length === 0}
		<div
			class="rounded-lg border border-dashed border-[var(--color-tron-border)] p-8 text-center text-[var(--color-tron-text-secondary)]"
		>
				...
				>
						...
						{#if img.inspection}
							<div class="absolute right-1 top-1">
								{#if img.inspection.result === 'pass'}
									<span
										class="rounded-full bg-[var(--color-tron-green)] px-1.5 py-0.5 text-xs font-bold text-white"
										>PASS</span
									>
								{:else if img.inspection.result === 'fail'}
									<span
										class="rounded-full bg-[var(--color-tron-red)] px-1.5 py-0.5 text-xs font-bold text-white"
										>FAIL</span
									>
								{:else}
									<span
										class="animate-pulse rounded-full bg-[var(--color-tron-yellow)] px-1.5 py-0.5 text-xs font-bold text-white"
										>{img.inspection.status}</span
									>
								{/if}
							</div>
						{/if}
					...
					<div class="p-2">
						{#if img.cartridge_tag?.phase}
							<span
								class="inline-block rounded bg-[var(--color-tron-bg-tertiary)] px-1.5 py-0.5 text-xs text-[var(--color-tron-text-secondary)]"
							>
						...
						{#if img.cartridge_tag?.labels?.length}
							<div class="mt-1 flex flex-wrap gap-0.5">
								{#each img.cartridge_tag.labels.slice(0, 3) as label}
									<span
										class="rounded-full border border-[var(--color-tron-border)] px-1.5 py-0 text-[10px] text-[var(--color-tron-text-secondary)]"
										>{label}</span
									>
								{/each}
							</div>
		...
		>
			...
			<div class="p-4">
				<div class="flex items-center gap-3">
					{#if lightboxImage.inspection}
						{#if lightboxImage.inspection.result === 'pass'}
							<span
								class="rounded-full bg-[var(--color-tron-green)]/20 px-3 py-1 text-sm font-bold text-[var(--color-tron-green)]"
								>PASS</span
							>
						{:else if lightboxImage.inspection.result === 'fail'}
							<span
								class="rounded-full bg-[var(--color-tron-red)]/20 px-3 py-1 text-sm font-bold text-[var(--color-tron-red)]"
								>FAIL</span
							>
						{/if}
				...
				{#if lightboxImage.inspection?.defects?.length}
					<div class="mt-3">
						<p class="text-xs uppercase tracking-wider text-[var(--color-tron-text-secondary)]">
							Defects
						</p>
				...
				{#if lightboxImage.cartridge_tag?.labels?.length}
					<div class="mt-3 flex flex-wrap gap-1">
						{#each lightboxImage.cartridge_tag.labels as label}
							<span
								class="rounded-full border border-[var(--color-tron-cyan)]/30 px-2 py-0.5 text-xs text-[var(--color-tron-cyan)]"
								>{label}</span
							>
						{/each}
					</div>
