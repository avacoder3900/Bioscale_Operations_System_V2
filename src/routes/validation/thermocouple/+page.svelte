<script lang="ts">
	...
	// Order by when the run actually happened. startedAt is the run's own start
	// time (completedAt tracks the upload into BIMS, which can be much later), so
	// a run with no startedAt sorts to the bottom rather than borrowing one.
	function runDate(s: { startedAt: string | null }): number | null {
		return s.startedAt ? new Date(s.startedAt).getTime() : null;
	}
	...
	function exportToCsv() {
		const headers = ['UDI', 'Run Number', 'Date', 'User', 'Status', 'Min Temp (°C)', 'Max Temp (°C)', 'Avg Temp (°C)', 'Result'];
...
<svelte:head>
	<title>Thermocouple Validation | Bioscale</title>
</svelte:head>
...
<div class="space-y-6">
	...
	<div class="flex items-center justify-between">
		...
		<a href="/validation/thermocouple/run" class="tron-btn-primary">Run a test</a>
	...
	<div class="tron-card overflow-hidden">
		{#if data.sessions.length === 0}
			<div class="p-8 text-center">
				...
				<a href="/validation/thermocouple/run" class="mt-4 inline-block text-[var(--color-tron-cyan)] hover:underline">
					Run a test →
				</a>
			</div>
		{:else}
			<table class="w-full">
				<thead class="border-b border-[var(--color-tron-border)] bg-[var(--color-tron-bg-tertiary)]">
					<tr>
						...
						<th class="tron-text-muted px-4 py-3 text-left text-xs font-medium uppercase">Run Number</th>
					</tr>
				</thead>
					...
					{#each sessions as session (session.id)}
						...
						<tr class="transition-colors hover:bg-[var(--color-tron-bg-tertiary)]">
							...
							<td class="tron-text-secondary px-4 py-3 text-sm whitespace-nowrap">
								...
								{:else}
									<span class="tron-text-muted" title="No run start time recorded">—</span>
								{/if}
							</td>
							...
							<td class="tron-text-secondary px-4 py-3 font-mono text-sm">
								{session.runNumber ?? '—'}
							</td>
						</tr>
					{/each}
