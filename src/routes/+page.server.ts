		...
		cartridgeDashboard: await (async () => {
			try {
				const [statusCounts, typeCounts, groups, recentActivity] = await Promise.all([
					...
					LabCartridge.find().sort({ updatedAt: -1 }).limit(20).lean()
				]);
				...
				return {
					...
					recentActivity: recentActivity.map((c: any) => ({
						id: c._id, barcode: c.barcode, status: c.status,
						cartridgeType: c.cartridgeType, updatedAt: c.updatedAt
					})),
