/**
 * /manufacturing/cart-mfg/wax-inspect — inline CV deployment point (WAX-FLOW-4).
 *
 * Every cartridge coming off wax filling gets photographed here and the
 * deployed model's verdict is shown to the operator immediately. The page is
 * a phase-pinned ('wax_filled') sibling of /capture: same scanner-wedge sticky
 * context, same Pi-station WebRTC + USB-camera capture paths, same
 * POST /api/cv/capture → poll /api/cv/inspections round-trip.
 *
 * Server load supplies:
 *   - stations:          all CaptureStations (status-badged; tokens fetched on demand)
 *   - modelDeployed:     does ANY CvProject deploy at 'wax_filled' with an active model?
 *   - deployedProjects:  name/version of those projects for the header display
 *   - recentInspections: last 50 wax_filled CvInspections joined with CvImage thumbnails
 */
...
export const load: PageServerLoad = async ({ locals }) => {
	...
	// Is anything actually deployed at wax_filled? Drives the yellow
