...
<div class="min-h-screen tron-grid-bg p-4 lg:p-6">
	<div class="mb-6 flex flex-wrap items-center justify-between gap-3">
		...
		<div class="flex items-center gap-2">
			...
			<a href="/manufacturing/cart-mfg/pipeline?stage={data.stage}" class="rounded border border-[var(--color-tron-border)] px-3 py-1.5 text-xs text-tron-text-secondary hover:text-tron-cyan hover:border-tron-cyan/50">
				⟳ Refresh
			</a>
		</div>
	</div>
		...
		<div class="flex flex-wrap items-center gap-3">
			...
			<div class="ml-auto flex items-center gap-1 text-xs text-tron-text-secondary">
				{#each data.stages as s, i (s.key)}
					<a
						href="/manufacturing/cart-mfg/pipeline?stage={s.key}"
