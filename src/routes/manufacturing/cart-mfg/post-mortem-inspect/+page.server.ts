 ...
 * (POST-MORTEM-INSPECT).
 *
 * Runs AFTER a cartridge has been ran (status `completed`): every ran cartridge can
 * be photographed here and the deployed model's verdict is shown immediately. Mirror
 * of /reagent-inspect, pinned to the 'post_mortem' phase: same scanner-wedge sticky
 * context, same Pi-station WebRTC + USB-camera capture paths, same POST /api/cv/capture
 * → poll /api/cv/inspections round-trip. The scan-gated human verdict moves a cart
 * postmortem_qc → postmortem_ready | postmortem_rejected via /api/cv/postmortem-verdict.
 *
