<script lang="ts">
	 ...
	 * model's verdict lands. The scan-gated human verdict moves a postmortem_qc cart →
	 * postmortem_ready / postmortem_rejected via /api/cv/postmortem-verdict.
	...
	// completed (ran) carts get photographed (→ postmortem_qc); postmortem_qc carts
	// get re-scanned to give the Accept/Reject verdict.
	const ALLOWED_STATUSES = ['completed', 'postmortem_qc'];
	...
	async function capturePhoto() {
		...
		try {
			...
			// Photographing a ran (completed) cart advanced it to postmortem_qc
			// server-side — reflect that so the Accept/Reject verdict buttons appear.
			if (cartridgeStatus === 'completed') cartridgeStatus = 'postmortem_qc';
	...
	// ── Post-mortem verdict (scan-gated): postmortem_qc → postmortem_ready / postmortem_rejected ──
	// The scanned (sticky-context) cartridge IS the physical-scan gate. Only a
	// postmortem_qc cart (photographed, no verdict yet) can be judged. CV auto-verdict
	// will reuse the same /api/cv/postmortem-verdict endpoint with source:'cv'.
	...
	async function submitVerdict(v: 'ready' | 'rejected') {
		...
		let reason = '';
		if (v === 'rejected') {
			reason = (prompt('Reason for rejecting this ran cartridge?') ?? '').trim();
			if (!reason) return; // cancelled / empty
		}
		verdictBusy = true;
		verdictMsg = null;
		try {
			const res = await fetch('/api/cv/postmortem-verdict', {
				method: 'POST',
			...
			if (!res.ok || body.error) {
				...
				cartridgeStatus = body.status; // postmortem_ready | postmortem_rejected
...
<div class="min-h-screen bg-[var(--color-tron-bg-primary)] p-4 sm:p-6">
	<div class="mx-auto max-w-6xl space-y-4">
		<header class="flex items-center justify-between">
			<div>
				...
				<p class="text-xs text-[var(--color-tron-text-secondary)]">
					Scan each cartridge that has been ran, press Space to photograph it, and the deployed model's PASS/FAIL verdict appears below. The verdict is advisory — the post-mortem accept/reject (scan-gated) stays with you.
				</p>
			</div>
		...
		<div class="flex items-center justify-between gap-3">
			...
			<!-- Scan-gated verdict: a photographed (postmortem_qc) cart gets passed/failed.
			     The scanned sticky-context cartridge IS the physical-scan gate. -->
			{#if cartridgeId && cartridgeStatus === 'postmortem_qc'}
				<div class="flex flex-col items-center gap-2 border-l border-[var(--color-tron-border)] pl-4">
					<div class="text-xs text-[var(--color-tron-text-secondary)]">Post-mortem inspection verdict</div>
			...
			{:else if cartridgeId && (cartridgeStatus === 'postmortem_ready' || cartridgeStatus === 'postmortem_rejected')}
				<div class="text-sm font-semibold {cartridgeStatus === 'postmortem_ready' ? 'text-green-400' : 'text-red-400'}">
