...
export const actions: Actions = {
	 ...
	 */
	/**
	 * Mid-run tip swap. Asks the on-robot bridge daemon to write a request file
	 * that the running reagent protocol polls before every dispense. The protocol
	 * then gives the un-dispensed reagent back to the source tube, swaps the tip
	 * (mode 'rack' = robot takes the next tracked tip; 'hand' = raises the pipette
	 * over the calibrator and pauses for the operator to push one on), re-probes it
	 * on the calibrator, and re-aspirates + continues at the very well it was about
	 * to fill. Works whether the run is running or paused.
	 *
	 * Mirrors the wax fill's requestTipSwap — same bridge command kind and same
	 * request file, so one robot only ever has one pending swap.
	 */
	requestTipSwap: async ({ request, locals }) => {
		...
		await AuditLog.create({
			...
			resourceType: 'reagent_batch_record',
			resourceId: runId,
