<script lang="ts">
	 ...
	 * reagent_qc cart → reagent_ready / reagent_rejected via /api/cv/reagent-verdict.
	...
	async function handleScan(rawCode: string) {
		...
		try {
			...
			if (!status || !ALLOWED_STATUSES.includes(status)) {
				rejectBanner = `Cartridge ${info.cartridgeRecordId ?? code} has status "${status ?? 'unknown'}" — reagent inspect accepts only: ${ALLOWED_STATUSES.join(', ')}.`;
				cartridgeId = null;
				cartridgeStatus = null;
				return;
			}
		...
		try {
			...
			if (track) {
				try {
					...
					console.warn('[reagent-inspect] LIZA tuning applyConstraints skipped:', e);
				}
			}
	...
	// ── Reagent verdict (scan-gated): reagent_qc → reagent_ready / reagent_rejected ──
	// The scanned (sticky-context) cartridge IS the physical-scan gate. Only a
	// reagent_qc cart (photographed, no verdict yet) can be judged. CV auto-verdict
	// will reuse the same /api/cv/reagent-verdict endpoint with source:'cv'.
	...
	async function submitVerdict(v: 'ready' | 'rejected') {
		...
		let reason = '';
		if (v === 'rejected') {
			reason = (prompt('Reason for rejecting this reagent cartridge?') ?? '').trim();
			if (!reason) return; // cancelled / empty
		}
		verdictBusy = true;
		verdictMsg = null;
		try {
			const res = await fetch('/api/cv/reagent-verdict', {
				method: 'POST',
			...
			if (!res.ok || body.error) {
				...
				cartridgeStatus = body.status; // reagent_ready | reagent_rejected
	...
	<div class="mx-auto max-w-6xl space-y-4">
		...
		{:else}
			<div class="rounded border border-[var(--color-tron-green,#39ff14)] bg-[rgba(57,255,20,0.05)] p-2 text-xs">
				...
				<span class="font-mono text-[var(--color-tron-cyan)]">reagent_filled</span>
		...
		<div class="flex items-center justify-between gap-3">
			...
			<!-- Scan-gated verdict: a photographed (reagent_qc) cart gets passed/failed.
			     The scanned sticky-context cartridge IS the physical-scan gate. -->
			{#if cartridgeId && cartridgeStatus === 'reagent_qc'}
				<div class="flex flex-col items-center gap-2 border-l border-[var(--color-tron-border)] pl-4">
					<div class="text-xs text-[var(--color-tron-text-secondary)]">Reagent inspection verdict</div>
			...
			{:else if cartridgeId && (cartridgeStatus === 'reagent_ready' || cartridgeStatus === 'reagent_rejected')}
				<div class="text-sm font-semibold {cartridgeStatus === 'reagent_ready' ? 'text-green-400' : 'text-red-400'}">
		...
		<!-- Session feed: server-loaded 50 recent reagent_filled inspections + live prepends -->
		<div>
			<h3 class="mb-2 text-sm font-semibold uppercase text-[var(--color-tron-text-secondary)]">Recent reagent_filled inspections</h3>
			{#if feed.length === 0}
