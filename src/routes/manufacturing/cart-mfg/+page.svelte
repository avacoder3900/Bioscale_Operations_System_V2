...
<div class="space-y-6 p-4 lg:p-6">
	...
	<div class="flex items-center justify-between gap-3">
		...
		<nav class="flex items-center gap-2 text-xs">
			<a
				href="/manufacturing/cart-mfg/wax-filling/settings"
			...
			<a
				href="/manufacturing/cart-mfg/reagent-filling/settings"
				class="rounded border border-[var(--color-tron-border)] px-3 py-1.5 text-[var(--color-tron-text-secondary)] transition-colors hover:border-[var(--color-tron-cyan)] hover:text-[var(--color-tron-cyan)]"
			>
		...
		{#each data.robots as robot}
			<div class="rounded-lg border border-[var(--color-tron-border)] bg-[var(--color-tron-bg-secondary)] border-l-4 {robotStatusBorder(robot.status)} p-4">
				...
				{#if robot.activeWaxRun}
					...
					<a href="/manufacturing/cart-mfg/wax-filling?robot={robot.robotId}" class="mt-3 block text-xs text-[var(--color-tron-cyan)] hover:underline">→ Go to Wax Filling</a>
				{:else if robot.activeReagentRun}
					...
					<a href="/manufacturing/cart-mfg/reagent-filling?robot={robot.robotId}" class="mt-3 block text-xs text-[var(--color-tron-cyan)] hover:underline">→ Go to Reagent Filling</a>
				{:else}
					<div class="text-xs text-[var(--color-tron-text-secondary)] mb-2">No active run</div>
					<div class="flex gap-3">
						<a href="/manufacturing/cart-mfg/wax-filling?robot={robot.robotId}" class="text-xs text-[var(--color-tron-cyan)] hover:underline">▶ Start Wax</a>
						<a href="/manufacturing/cart-mfg/reagent-filling?robot={robot.robotId}" class="text-xs text-[var(--color-tron-cyan)] hover:underline">▶ Start Reagent</a>
					</div>
				{/if}
			</div>
		{/each}
