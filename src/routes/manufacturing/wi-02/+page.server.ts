/**
 * WI-02: Cut Thermoseal
 * Lot traceability: Input lot (thermoseal roll barcode)
 * → Output lot (LotRecord.qrCodeRef, LOT-YYYYMMDD-XXXX)
 * ISO 13485: thermoseal roll → cut strips batch lot
 */
...
export const load: PageServerLoad = async ({ locals }) => {
	...
	if (!config) {
		return {
			...
			recentLots: [],
			inventory: {
				input: { name: 'Thermoseal Roll', quantity: 0, unit: 'rolls' },
				output: { name: 'Cut Thermoseal Strips', quantity: 0, unit: 'strips' }
			},
			error: 'No cut thermoseal process configuration found'
		};
	}
	...
	return {
		config: {
			...
			processName: c.processName ?? 'Cut Thermoseal (WI-02)',
			maxBatchSize: c.maxBatchSize ?? 500,
			handoffPrompt: c.handoffPrompt ?? 'Cut thermoseal strips ready for laser cutting.',
		...
		inventory: {
			input: { name: 'Thermoseal Roll', quantity: findQty(/thermoseal.?roll|rol.?thermoseal/i), unit: 'rolls' },
			output: { name: 'Cut Thermoseal Strips', quantity: findQty(/thermoseal.?strip|cut.?thermoseal/i), unit: 'strips' }
		}
	};
};
...
export const actions: Actions = {
	...
	/** Record the input lot (thermoseal roll barcode) */
	setInputLots: async ({ request, locals }) => {
		...
		if (!barcode) return fail(400, { setInputLots: { error: 'Thermoseal roll lot/barcode required' } });

		const inputLots = [{
			materialName: 'Thermoseal Roll',
			barcode,
			scanOrder: 1,
			scannedAt: new Date()
		}];
	...
	finishBatch: async ({ request, locals }) => {
		...
		return {
			finishBatch: {
				success: true,
				lotId,
				handoffPrompt: config?.handoffPrompt ?? 'Cut thermoseal strips ready for laser cutting.'
			}
		};
	},
