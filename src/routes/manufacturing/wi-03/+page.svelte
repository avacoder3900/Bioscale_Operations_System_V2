		...
		<div class="rounded-lg border border-[var(--color-tron-border)] bg-[var(--color-tron-bg-secondary)] p-4">
			{#if step === 'qr'}
				<form method="POST" action="?/bindQR" use:enhance>
					...
					{/if}
					<button type="submit" class="mt-3 rounded bg-[var(--color-tron-cyan)] px-4 py-2 text-white">Bind QR</button>
				</form>
			...
			{:else if step === 'inputs'}
				<form method="POST" action="?/setInputLots" use:enhance>
					...
					<label class="mt-3 block text-sm text-[var(--color-tron-text)]">{data.config.inputMaterials[0].name}</label>
					<div class="mt-1 flex gap-2">
						...
						<button type="button" onclick={generateInput} class="rounded border border-[var(--color-tron-border)] px-3 py-2 text-xs text-[var(--color-tron-text-secondary)] hover:border-[var(--color-tron-orange)] hover:text-[var(--color-tron-orange)]" title="Generate test barcode">Test</button>
					</div>
					...
					{/if}
					<button type="submit" class="mt-3 rounded bg-[var(--color-tron-cyan)] px-4 py-2 text-white">Confirm input</button>
				</form>
			{:else if step === 'start'}
