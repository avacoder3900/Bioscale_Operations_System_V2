	...
	interface Props {
		data: {
			...
			inventory: {
				...
				output: { name: string; quantity: number; unit: string };
			};
			error?: string;
		};
		form: {
	...
	async function startBarcodeDetection() {
		...
		detectionInterval = setInterval(async () => {
			...
			try {
				...
				if (barcodes.length > 0) handleCameraScan(barcodes[0].rawValue);
			} catch { /* ignore */ }
		}, 300);
	}
...
{#if cameraOpen}
	...
	<div class="camera-overlay" onclick={(e) => e.stopPropagation()}>
		...
		<div class="mt-6 flex gap-4">
			<button type="button" class="touch-target rounded-lg border border-[var(--color-tron-border)] px-6 text-[var(--color-tron-text-secondary)]" onclick={closeCameraAndClean}>Cancel</button>
		</div>
	</div>
{/if}
	...
	<div class="space-y-6">
		...
		<div class="grid grid-cols-2 gap-4">
			<div class="rounded-lg border border-[var(--color-tron-border)] bg-[var(--color-tron-surface)] p-4">
				<p class="text-xs font-medium text-[var(--color-tron-text-secondary)]">Input: {data.inventory.input.name}</p>
				<p class="mt-1 text-2xl font-bold text-[var(--color-tron-text)]">
					{data.inventory.input.quantity}
					<span class="text-sm font-normal text-[var(--color-tron-text-secondary)]">{data.inventory.input.unit}</span>
				</p>
			</div>
			<div class="rounded-lg border border-[var(--color-tron-border)] bg-[var(--color-tron-surface)] p-4">
				<p class="text-xs font-medium text-[var(--color-tron-text-secondary)]">Output: {data.inventory.output.name}</p>
				<p class="mt-1 text-2xl font-bold text-[var(--color-tron-cyan)]">
					{data.inventory.output.quantity}
					<span class="text-sm font-normal text-[var(--color-tron-text-secondary)]">{data.inventory.output.unit}</span>
				</p>
			</div>
		</div>
		...
		<div class="rounded-lg border border-[var(--color-tron-border)] bg-[var(--color-tron-bg-secondary)] p-4">
			{#if step === 'qr'}
				<form method="POST" action="?/bindQR" use:enhance>
					...
					<div class="mt-2 flex gap-2">
						...
						/>
						<button type="button" onclick={() => openCamera('qr')} class="flex min-h-[56px] min-w-[56px] items-center justify-center rounded border border-[var(--color-tron-cyan)] bg-[rgba(0,212,255,0.08)] text-[var(--color-tron-cyan)]" aria-label="Scan with camera">
							<svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
								<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" />
								<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" />
							</svg>
						</button>
						<button type="button" onclick={() => { generateQR(); feedback('success'); }} class="hidden md:flex items-center rounded border border-[var(--color-tron-border)] px-3 text-xs text-[var(--color-tron-text-secondary)] hover:border-[var(--color-tron-orange)] hover:text-[var(--color-tron-orange)]" title="Generate test QR">Test</button>
					</div>
					...
					{/if}
					<button type="submit" class="mt-3 w-full rounded bg-[var(--color-tron-cyan)] font-bold text-[var(--color-tron-bg-primary)]" style="min-height: 56px;" onclick={() => feedback('success')}>Bind QR</button>
				</form>
			...
			{:else if step === 'inputs'}
				<form method="POST" action="?/setInputLots" use:enhance>
					...
					<div>
						...
						<div class="flex gap-2">
							<input
								...
								style="min-height: 56px; font-size: 1.125rem;"
							/>
							<button type="button" onclick={() => openCamera('input1')} class="flex min-h-[56px] min-w-[56px] items-center justify-center rounded border border-[var(--color-tron-cyan)] bg-[rgba(0,212,255,0.08)] text-[var(--color-tron-cyan)]" aria-label="Scan with camera">
								<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" />
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" />
								</svg>
							</button>
							<button type="button" onclick={() => { generateInput(); feedback('success'); }} class="hidden md:flex items-center rounded border border-[var(--color-tron-border)] px-3 text-xs text-[var(--color-tron-text-secondary)] hover:border-[var(--color-tron-orange)] hover:text-[var(--color-tron-orange)]" title="Generate test">Test</button>
						</div>
					</div>
					...
					{/if}
					<button type="submit" class="mt-4 w-full rounded bg-[var(--color-tron-cyan)] font-bold text-[var(--color-tron-bg-primary)]" style="min-height: 56px;" onclick={() => feedback('success')}>Confirm Input</button>
				</form>
			{:else if step === 'start'}
				<form method="POST" action="?/startBatch" use:enhance>
					<input type="hidden" name="lotId" value={lotId} />
					<p class="text-sm text-[var(--color-tron-text)]">Input set. Press Start to begin batch.</p>
					<button type="submit" class="mt-3 w-full rounded bg-[var(--color-tron-cyan)] font-bold text-[var(--color-tron-bg-primary)]" style="min-height: 56px;" onclick={() => feedback('success')}>Start Batch</button>
				</form>
			{:else if step === 'work'}
				<div class="space-y-4">
						...
						<div class="md:hidden space-y-3">
							<div class="step-dots">
								{#each data.processSteps as psd, i (psd.id)}
									<button type="button" class="step-dot {i === mobileActiveStep ? 'active' : ''} {entriesForStep(psd.id).length > 0 ? 'completed' : ''}" onclick={() => (mobileActiveStep = i)} aria-label="Step {psd.stepNumber}"></button>
								{/each}
							</div>
							{#each data.processSteps.slice(mobileActiveStep, mobileActiveStep + 1) as mps (mps.id)}
								<div class="rounded-lg border border-[var(--color-tron-border)] bg-[var(--color-tron-bg-primary)] p-4">
									...
									{#if mps.description}<p class="mt-2 text-base leading-relaxed text-[var(--color-tron-text-secondary)] whitespace-pre-wrap">{mps.description}</p>{/if}
									{#if mps.imageUrl}<img src={mps.imageUrl} alt="Step {mps.stepNumber}" class="mt-3 w-full max-h-64 rounded border border-[var(--color-tron-border)] object-contain" />{/if}
									...
									{:else}
										<button type="button" onclick={() => { activeNoteStep = mps.id; noteText = ''; }} class="mt-3 flex min-h-[44px] w-full items-center justify-center gap-2 rounded border border-dashed border-[var(--color-tron-border)] text-sm text-[var(--color-tron-cyan)]">
											<svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>Add Note / Photo
										</button>
									{/if}
								</div>
							{/each}
							<div class="flex items-center gap-3">
								<button type="button" onclick={mobilePrevStep} disabled={mobileActiveStep === 0} class="flex flex-1 min-h-[56px] items-center justify-center gap-2 rounded-lg border border-[var(--color-tron-border)] font-medium text-[var(--color-tron-text-secondary)] disabled:opacity-30 hover:border-[var(--color-tron-cyan)] hover:text-[var(--color-tron-cyan)]">
								...
								<button type="button" onclick={mobileNextStep} disabled={mobileActiveStep >= data.processSteps.length - 1} class="flex flex-1 min-h-[56px] items-center justify-center gap-2 rounded-lg border border-[var(--color-tron-cyan)] bg-[rgba(0,212,255,0.08)] font-medium text-[var(--color-tron-cyan)] disabled:opacity-30">
						...
						<div class="hidden md:block space-y-4">
							{#each data.processSteps as ps (ps.id)}
								<div class="rounded-lg border border-[var(--color-tron-border)] bg-[var(--color-tron-bg-primary)] p-4">
									...
									{#if ps.description}<p class="mt-2 text-sm text-[var(--color-tron-text-secondary)] whitespace-pre-wrap">{ps.description}</p>{/if}
									{#if ps.imageUrl}<img src={ps.imageUrl} alt="Step {ps.stepNumber}" class="mt-2 max-h-48 rounded border border-[var(--color-tron-border)]" />{/if}
					...
					<div class="rounded-lg border border-[var(--color-tron-cyan)] bg-[var(--color-tron-bg-primary)] p-4">
						<form method="POST" action="?/finishBatch" use:enhance>
							...
							<div class="mb-3">
								...
								<input type="number" name="quantity" bind:value={quantity} min="1" max={data.config.maxBatchSize} class="w-full rounded border border-[var(--color-tron-border)] bg-[var(--color-tron-bg-primary)] px-3 font-mono text-lg text-[var(--color-tron-text)]" style="min-height: 56px;" />
							</div>
							...
							{/if}
							<button type="submit" class="w-full rounded bg-[var(--color-tron-cyan)] font-bold text-[var(--color-tron-bg-primary)]" style="min-height: 56px; font-size: 1.125rem;" onclick={() => feedback('success')}>Finish Batch</button>
						</form>
					</div>
				</div>
			{/if}
