...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const spus = await Spu.find(
		{ 'particleLink.particleDeviceId': { $exists: true, $ne: null } },
		{
			...
			status: 1,
			'validation.magnetometer.status': 1,
			'validation.magnetometer.completedAt': 1
		}
	...
	return {
		spus: spus.map((s: any) => ({
			...
			status: s.status,
			// 'pending' (schema default) or missing means no regular mag run has been
			// recorded yet — the sweep button gates on this.
			magStatus: s.validation?.magnetometer?.status ?? 'pending',
			magCompletedAt: s.validation?.magnetometer?.completedAt
				? new Date(s.validation.magnetometer.completedAt).toISOString()
				: null
		})),
