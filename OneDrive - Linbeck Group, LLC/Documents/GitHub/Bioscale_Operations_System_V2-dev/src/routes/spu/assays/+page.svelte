<script lang="ts">
	...
	function applyFilters() {
		...
		goto(`/spu/assays?${params.toString()}`);
	}
...
<div class="mx-auto max-w-7xl space-y-6 p-4">
	...
	<div class="flex items-center justify-between">
		...
		<div class="flex gap-2">
			<a
				href="/spu/assays/export?format=csv"
				class="tron-button"
	...
	<div class="tron-card p-4">
		<form onsubmit={handleSearch} class="flex flex-wrap items-end gap-3">
			...
			{#if data.filters.search || data.filters.status}
				<a href="/spu/assays" class="tron-button" style="min-height: 44px; opacity: 0.7">
					Clear
				</a>
			{/if}
		</form>
				...
				<tbody>
					{#each data.assays as assay (assay.assayId)}
						<tr
							...
							onclick={() => goto(`/spu/assays/${assay.assayId}`)}
						>
