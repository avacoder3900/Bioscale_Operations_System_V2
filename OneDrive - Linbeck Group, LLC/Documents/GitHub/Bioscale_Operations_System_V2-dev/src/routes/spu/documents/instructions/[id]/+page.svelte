<script lang="ts">
	...
	$effect(() => {
		if (form?.success && form.runId && form.workInstructionId) {
			goto(`/spu/documents/instructions/${form.workInstructionId}/run/${form.runId}`);
		}
	});
	...
	async function checkRunInventory() {
		...
		try {
			const res = await fetch('/spu/documents/instructions/check-inventory', {
				method: 'POST',
	...
	<div class="flex items-start justify-between">
		<div>
			<div class="flex items-center gap-3">
				<a
					href={resolve('/spu/documents/instructions')}
		...
		<div class="flex items-center gap-2">
			...
			{/if}
			<a
				href={resolve(`/spu/documents/instructions/${$page.params.id}/fields`)}
				...
				<div class="run-section {selectMode && selectedRunIds.has(run.id) ? 'run-section-selected' : ''}">
					<div class="run-header">
						...
						{#if data.isAdmin && !selectMode}
							<a
								href="/spu/documents/instructions/{$page.params.id}/run/{run.id}?cancel=true"
					...
					<div class="unit-grid">
						{#each run.units as unit (unit.unitId)}
							<button
								...
								onclick={() =>
									goto(
										`/spu/documents/instructions/${$page.params.id}/run/${run.id}`
									)}
							>
