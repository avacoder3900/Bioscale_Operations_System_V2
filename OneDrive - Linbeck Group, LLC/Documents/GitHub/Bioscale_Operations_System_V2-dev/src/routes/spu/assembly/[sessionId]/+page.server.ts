...
export const actions: Actions = {
	...
	scanPart: async ({ request, locals, params }) => {
		...
		// Determine withdrawal quantity from work instruction step partRequirements
		let withdrawQty = 1;
		let matchedBomItemId: string | null = null;
		let stepRecordId: string | null = null;

		if (workInstructionStepId && s.workInstructionId) {
			const wi = await WorkInstruction.findById(s.workInstructionId).lean() as any;
			if (wi) {
				const currentVersion = (wi.versions ?? []).find(
					(v: any) => v.version === wi.currentVersion
				) ?? (wi.versions ?? []).slice(-1)[0];
				const wiStep = (currentVersion?.steps ?? []).find(
					(step: any) => step._id === workInstructionStepId
				);
				if (wiStep?.partRequirements?.length) {
					// Find matching part requirement by partNumber or partDefinitionId
					const matchedReq = wiStep.partRequirements.find((pr: any) =>
						(pr.partDefinitionId && pr.partDefinitionId === partDefinitionId) ||
						(pr.partNumber && partDef && pr.partNumber === partDef.partNumber)
					);
					if (matchedReq) {
						withdrawQty = matchedReq.quantity ?? 1;
					} else if (partDef) {
						// Scanned part doesn't match any requirement on this step
						return fail(400, {
							error: `Part ${partDef.partNumber} does not match any part requirement for this step. Expected: ${wiStep.partRequirements.map((pr: any) => pr.partNumber).join(', ')}`
						});
					}
				}
			}

			// Look up linked BOM item for traceability
			if (partDef) {
				const bomItem = await BomItem.findOne({
					partNumber: partDef.partNumber, bomType: 'spu', isActive: true
				}).lean() as any;
				if (bomItem) matchedBomItemId = bomItem._id;
			}
		}
		...
		if (workInstructionStepId) {
			...
			if (!existingRecord) {
				...
				s.stepRecords.push({ _id: newRecordId, workInstructionStepId, stepNumber: s.stepRecords.length + 1, scannedLotNumber: lotNumber, scannedPartNumber: partDef?.partNumber ?? '', completedAt: new Date(), completedBy: { _id: locals.user!._id, username: locals.user!.username } });
				stepRecordId = newRecordId;
			} else {
				stepRecordId = existingRecord._id;
			}
		} else {
		...
		// Inventory deduction with correct quantity
		...
		if (s.spuId) {
			...
			if (partDefinitionId && partDef) {
				...
				if (prevQty < withdrawQty) {
					return fail(400, {
						error: `Insufficient inventory for ${partDef.partNumber}. Need ${withdrawQty}, have ${prevQty}.`
					});
				}
				const newQty = prevQty - withdrawQty;
				await PartDefinition.updateOne({ _id: partDefinitionId }, { $inc: { inventoryCount: -withdrawQty } });
				await InventoryTransaction.create({
					_id: generateId(),
					partDefinitionId,
					bomItemId: matchedBomItemId,
					assemblySessionId: params.sessionId,
					...
					newQuantity: newQty,
					reason: `Assembly scan for SPU ${s.spuId} (qty: ${withdrawQty})`,
					performedBy: locals.user!.username,
					performedAt: new Date()
				});
				inventoryDeduction = { previousQuantity: prevQty, newQuantity: newQty, quantityDeducted: withdrawQty };
			}
		}
