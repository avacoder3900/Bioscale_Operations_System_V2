...
<div class="space-y-6">
	...
	<form method="GET" class="tron-card flex flex-wrap items-end gap-4 p-4">
		...
		{#if data.filters.status || data.filters.from || data.filters.to}
			<a href="/spu/validation/magnetometer/history" class="tron-text-muted text-sm hover:text-[var(--color-tron-cyan)]">
				Clear filters
			</a>
		{/if}
	</form>
	...
	<div class="tron-card overflow-hidden">
		{#if data.sessions.length === 0}
			<div class="p-8 text-center">
				...
				<a href="/spu/validation/magnetometer" class="mt-4 inline-block text-[var(--color-tron-cyan)] hover:underline">
					Run a test →
				</a>
			</div>
		{:else}
