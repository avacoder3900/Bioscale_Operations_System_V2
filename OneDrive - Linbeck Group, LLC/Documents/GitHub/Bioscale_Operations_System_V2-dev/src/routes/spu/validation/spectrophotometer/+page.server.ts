...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const sessions = await ValidationSession.find({ type: 'spec' })
		.sort({ createdAt: -1 })
		.limit(20)
		.lean();
...
export const actions: Actions = {
	start: async ({ locals }) => {
		...
		const barcode = `SPEC-${String(seq).padStart(6, '0')}`;
		...
		redirect(303, `/spu/validation/spectrophotometer/${sessionId}`);
	}
};
