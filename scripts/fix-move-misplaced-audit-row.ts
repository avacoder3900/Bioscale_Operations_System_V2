 ...
 * wrote via raw collection.insertOne to `audit_log` (plural). The Mongoose
...
async function main() {
	...
	const wrong = await db.collection('audit_log').find({ action: 'RECONCILE' }).toArray();
	console.log(`Rows in WRONG collection (audit_log plural): ${wrong.length}`);
	...
	const result = await db.collection('audit_log').deleteMany({ _id: { $in: ids } });
	console.log(`✓ Removed ${result.deletedCount} row(s) from audit_log (plural)`);
