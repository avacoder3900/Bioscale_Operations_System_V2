/**
 * Deploy the working-tree protocols/Wax_Filling_GEN7_Cartridge.py (tip-calibration
 ...
 *
 *   node scripts/deploy-wax-tipcal-guards.cjs                # B07, verify only
...
async function main() {
  ...
  const currentEntry = (robot.protocols ?? []).filter((p) => p.protocolType === 'wax-filling').sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0))[0];
  if (!currentEntry) throw new Error(`${R.name} has no wax-filling protocol entry`);
  ...
  const pyBuf = fs.readFileSync(path.join(REPO, 'protocols', 'Wax_Filling_GEN7_Cartridge.py'));
  console.log(`Protocol: working tree Wax_Filling_GEN7_Cartridge.py (${(pyBuf.length / 1024).toFixed(1)} KB)`);
  ...
  const before = (robot.protocols ?? []).filter((p) => p.protocolType === 'wax-filling').map((p) => p.opentronsProtocolId);
  console.log(`\n  ${R.name} wax entries BEFORE: ${JSON.stringify(before)}  (rollback target)`);
  await robots.updateOne({ _id: R.id }, { $pull: { protocols: { protocolType: 'wax-filling' } } });
  await robots.updateOne({ _id: R.id }, { $push: { protocols: {
    ...
    uploadedBy: 'deploy-wax-tipcal-guards', createdAt: new Date(), updatedAt: new Date()
  } } });
  await db.collection('audit_logs').insertOne({
    _id: 'al-' + Math.random().toString(36).slice(2, 18),
    action: 'wax_protocol_deploy', resourceType: 'opentrons_robot', resourceId: R.id,
    userId: 'script', username: 'deploy-wax-tipcal-guards', timestamp: new Date(),
